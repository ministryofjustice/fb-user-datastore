...
RSpec.describe DbSweeper do
  describe '#call UserData' do
    context 'when there is user data over 28 days old' do
      before :each do
        create(:user_data, created_at: 30.days.ago, updated_at: 30.days.ago)
        create(:user_data, created_at: 5.days.ago, updated_at: 5.days.ago)
      end
    ...
    context 'when the user data was updated' do
      context 'updated within threshold' do
        before do 
          create(:user_data, created_at: 30.days.ago, updated_at: 5.days.ago)
        end

        it 'does not destroy records returned to within 28 days' do
          expect do
            subject.call
          end.to_not change(UserData, :count)
        end
      ...
      context 'after threshold' do
        before do 
          create(:user_data, created_at: 45.days.ago, updated_at: 28.days.ago)
        end
